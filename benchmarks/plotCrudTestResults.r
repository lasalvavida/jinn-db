#!/usr/bin/env Rscript
jinnDb <- read.csv("results/crudTestJinnDb.csv", header=TRUE, sep=",")
nedb <- read.csv("results/crudTestNedb.csv", header=TRUE, sep=",")
# tingoDb <- read.csv("results/insertTestTingoDb.csv", header=TRUE, sep=",")

t_domain <- range(0, jinnDb$time, nedb$time)
...
title(ylab="# of Instructions Executed")
